...
// const prefixString = require('./lib/prefix-string');

// const createServer = async ({ serveDir, auditUrl }) => {
//   console.log('Initiating server', { serveDir, auditUrl });
//   execSync(`mv ${serveDir} ${serveDir}_temp`);
//   const server = execSync(
//     `netlify dev --port 52014 --dir ${serveDir}_temp > /dev/null 2>&1 &`,
//   );
//   let ready = false;
//   while (!ready) {
//     try {
//       console.log('Checking Server');
//       console.log(await fetch('http://localhost:52014'));
...
// }) => {
//   try {
//     const { server } = createServer({ serveDir: serveDir, auditUrl: url });
...
//         inputs,
//       });
//       const publishDir = constants.PUBLISH_DIR || process.env.PUBLISH_DIR;
...
//       execSync(`rm -fr ${publishDir}`);
//       execSync(`mv ${publishDir}_temp ${publishDir}`);
